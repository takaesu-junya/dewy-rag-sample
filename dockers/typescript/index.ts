...
program.name('dewy_qa').description('CLI tool for interacting with a knowledge base API').version('1.0.0')
...
program
    ...
    .option('--collection <collection>', 'Specify the collection name', 'main')
    .option('--dewy-endpoint <endpoint>', 'Specify the collection name', 'http://localhost:8000')
    .option('--openai-api-key <key>', 'Specify the api key for OpenAI api', defaultOpenAIKey)
